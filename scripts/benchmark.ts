...
class BuildTool {
  ...
  constructor({
    ...
    this.binFilePath = Array.isArray(binFilePath)
      ? resolveBinFilePath(path.join(process.cwd(), 'node_modules'), binFilePath)
      : path.join(process.cwd(), 'node_modules', binFilePath);
    this.hackBinFile();
    ...
    case 'rspack':
      buildTools.push(
        new BuildTool({
          ...
          binFilePath: ['@rspack/cli/bin/rspack.js', '@rspack/cli/bin/rspack'],
        }),
      );
      break;
    case 'rsbuild':
