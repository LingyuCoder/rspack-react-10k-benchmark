...
Minimal standalone benchmark repository for comparing `Rspack 1.0.0`, `1.7.11`, `2.0.0`, `latest`, and `latest` with `@rspack-canary/core` on the `react-10k` case.
...
- Memory cache scenario:
  ...
  - `HMR`
  - `Output size`
- Persistent cache scenario (`Rspack 1.7.11`, `2.0.0`, `latest`, and `latest` with `@rspack-canary/core` only):
  - `Build (no cache)`
